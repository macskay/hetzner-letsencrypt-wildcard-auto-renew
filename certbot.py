...
def renew(zone, record, domain):
    ...
    old = record["value"]
    child = pexpect.spawn(f"/bin/bash -c '{CERTBOT_PATH} certonly -d *.{domain} --server https://acme-v02.api.letsencrypt.org/directory --manual --preferred-challenges dns --manual-public-ip-logging-ok", encoding="utf-8")
